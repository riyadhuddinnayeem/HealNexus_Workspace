...
def get_patient_dashboard(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    appointments = db.query(Appointment).filter(Appointment.patient_id == current_user.id).order_by(Appointment.date_time.asc()).all()
    formatted_apts = []
    ...
    # Fetch Medical Vault Records (Ordered newest first)
    records = db.query(MedicalRecord).filter(MedicalRecord.patient_id == current_user.id).order_by(MedicalRecord.id.desc()).all()
...
# --- 2. BOOK AN APPOINTMENT WITH SYNCHRONIZED VAULT UPLOAD ---
...
def book_appointment(
    ...
    if file:
        ...
        # --- THE FIX: Synchronize with Medical Vault ---
        new_vault_record = MedicalRecord(
            patient_id=current_user.id,
            record_type="Appointment Attachment",
            description=f"Document shared for visit with Dr. {doctor.full_name} regarding: {reason}",
            file_path=file_path_for_db 
        )
        db.add(new_vault_record)
    ...
    db.commit()
    return {"message": "Appointment booked and document synced to vault successfully!"}


# --- 3. UPLOAD MEDICAL RECORD DIRECTLY ---
