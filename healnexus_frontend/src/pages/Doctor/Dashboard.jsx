...
import {
    ...
    CheckCircle, AlertCircle, Search, Video, Pill, Send, Loader2, User, Eye, X, ExternalLink, FileText
} from 'lucide-react';
...
export default function DoctorDashboard() {
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans relative">
                                ...
                                dashboardData.todays_appointments.map(app => (
                                    <div key={app.id} className="p-6 md:p-8 flex flex-col lg:flex-row lg:items-center justify-between hover:bg-slate-50 transition-colors gap-6">
                                        ...
                                        <div className="flex items-center gap-3">
                                            {/* THE FIX: Document Viewer Button for Doctors */}
                                            {app.document_path && (
                                                <button
                                                    onClick={() => setViewDocument(app.document_path)}
                                                    className="flex items-center px-4 py-2.5 bg-emerald-50 text-emerald-700 text-sm font-bold rounded-xl hover:bg-emerald-100 border border-emerald-200 transition-colors"
                    ...
                    <div className="animate-in fade-in">
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                            ...
                            <div className="relative w-full md:w-72">
                                ...
                                <input type="text" placeholder="Search patients..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-sm shadow-sm" />
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            {filteredPatients.length > 0 ? (
                                filteredPatients.map(patient => (
                                    <div key={patient.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex justify-between items-center hover:border-blue-300 transition-colors group cursor-pointer">
                                        <div className="flex items-center space-x-4">
                                            <div className="w-12 h-12 rounded-full bg-slate-100 text-slate-500 flex items-center justify-center font-bold border border-slate-200">{patient.name.charAt(0)}</div>
                                            <div>
                                ...
                                <div className="col-span-1 lg:col-span-2 text-center py-12 bg-white rounded-2xl border border-slate-200"><p className="text-slate-500">No patients found.</p></div>
                            )}
                        </div>
                    </div>
                )}
                        ...
                        {rxSuccess ? (
                            ...
                            <form onSubmit={handleSendPrescription} className="space-y-5">
                                <div>
                                    ...
                                    <select required value={rxForm.patient_id} onChange={(e) => setRxForm({ ...rxForm, patient_id: e.target.value })} className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none text-sm">
                                        <option value="">-- Choose from your patients --</option>
                                        {dashboardData.patients.map(p => <option key={p.id} value={p.id}>{p.name} (ID: {p.id})</option>)}
                                    </select>
                                </div>
            ...
            {/* --- NEW: DOCUMENT VIEWER MODAL FOR DOCTORS --- */}
            {viewDocument && (
                <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in">
                    <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl relative overflow-hidden">
                        ...
                        <div className="p-4 md:p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                            <div>
                                <h2 className="text-xl font-bold text-slate-900">Patient Attached Document</h2>
                            </div>
                            <button onClick={() => setViewDocument(null)} className="p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-full transition-colors">
                                <X className="w-6 h-6" />
                            </button>
                        </div>

                        <div className="flex-1 overflow-auto p-4 bg-slate-100 flex items-center justify-center min-h-[50vh]">
                            {viewDocument.match(/\.(jpeg|jpg|gif|png)$/i) ? (
                                <img src={`${API_BASE_URL}/${viewDocument}`} alt="Patient Record" className="max-w-full max-h-full object-contain rounded-lg shadow-sm" />
                            ) : viewDocument.match(/\.(pdf)$/i) ? (
                                <iframe src={`${API_BASE_URL}/${viewDocument}`} className="w-full h-[60vh] md:h-[70vh] rounded-lg shadow-sm bg-white" title="PDF Document" />
                            ) : (
                                <div className="text-center p-8 bg-white rounded-2xl shadow-sm border border-slate-200">
                                    ...
                                    <a href={`${API_BASE_URL}/${viewDocument}`} target="_blank" rel="noopener noreferrer" className="px-6 py-3 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition-colors inline-flex items-center">
